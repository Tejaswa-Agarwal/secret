      ...
      <div className="container" style={{ paddingTop: banner ? 0 : 40, paddingBottom: 60 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', gap: 32, marginTop: -120, position: 'relative', zIndex: 1, alignItems: 'start' }}>
          ...
          <div>
            ...
            {firstEp && (
              <Link
                href={`/watch/${numId}/${encodeURIComponent(firstEp.id)}`}
          ...
          <div style={{ paddingTop: 120 }}>
            ...
            {anime.title.native && (
              <p style={{ color: 'var(--text-muted)', fontSize: 13, marginBottom: 16 }}>{anime.title.native}</p>
            )}
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 20 }}>
              {anime.genres?.map(g => (
                <Link key={g} href={`/browse?genre=${encodeURIComponent(g)}`} className="genre-pill active" style={{ fontSize: 12 }}>
                  {g}
                </Link>
              ))}
            </div>
            ...
            {/* Description */}
            <p style={{ color: 'var(--text-secondary)', lineHeight: 1.7, fontSize: 14, maxWidth: 640, marginBottom: 32 }}>
              {description}
            </p>
